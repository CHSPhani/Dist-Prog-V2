...
namespace Server.ChangeRules
{
    public class ValidateRules
    {
        public static VoteType Validate(DBData nData, NodeMesaage nMsg)
        {
            ...
            else if (nMsg.PCause == ProposalCause.UploadInd)
            {
            ...
            else if (nMsg.PCause == ProposalCause.NewOClass)
            {
                ...
                string newBCls = nMsg.DataItems[1];
                
                //Check-1:Checking whether class name exists or not.
                var oc = nData.OwlData.RDFG.NODetails.Keys.ToList<string>().Find((o) => { if (o.Equals(newCls)) { return true; } else { return false; } });
                //Further Check-2: change the ontology (Graph -> Ontology) and execute Owlready2.Load() and then proceed further

                if (oc != null)
                {
                    return VoteType.NotAccepted; //Say No
                }
                return VoteType.Accepted; //say yes.
            }
            else
            {
